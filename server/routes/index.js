module.exports = (function(appConfig) {
  ...
  const log = require("../logger")("nqm-app:routes-index");
  ...
  const setupClient = require("./middlewares/frontendMiddleware");
  ...
  const setUserSession = function(req, res, redirectTo) {
    if (req.query.access_token) {
      ...
      if (decoded) {
        ...
        log.info("setUserSession: missing or invalid token received from auth server [%s]", req.query.access_token);
        res.redirect("/");
      }
    }
  };
  ...
  router.get("/nqm-setup-data", async function(req, res) {
    ...
    if ((!req.session || !req.session.token) && appConfig.publicShareKeyId && appConfig.publicShareKeySecret) {
      ...
      try {
        ...
        log.error("failed to get public share key token", err);
      }
    } else {
